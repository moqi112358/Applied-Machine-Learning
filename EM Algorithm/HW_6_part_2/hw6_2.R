...
pic_cluster <- function(picture, clusters,seed){
  ...
  for(i in 1:img_row){
    for(j in 1:img_col){
      ...
      final[i, j, ] <- segment_mean[, meanseg]*attr(mtx, 'scaled:scale') + attr(mtx, 'scaled:center')
    }
  }
  timeend<-Sys.time()
...
writeJPEG(pic_cluster(sky, 20, 0), "sky_segmented10.jpg",quality = 1)
writeJPEG(pic_cluster(sky, 20, 1), "sky_segmented10.jpg",quality = 1)
writeJPEG(pic_cluster(sky, 20, 2), "sky_segmented10.jpg",quality = 1)
writeJPEG(pic_cluster(sky, 20, 3), "sky_segmented10.jpg",quality = 1)
writeJPEG(pic_cluster(sky, 20, 4), "sky_segmented10.jpg",quality = 1)
